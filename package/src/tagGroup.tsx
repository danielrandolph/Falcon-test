  ...
  compoundVariants: [
    {
      ...
      isDisabled: false,
      className:
        "tw:border-blue-200 tw:bg-blue-100 tw:text-blue-700 tw:hover:border-blue-300",
    },
    {
      color: "gray",
      isSelected: false,
      isDisabled: false,
      className:
        "tw:border-neutral-200 tw:bg-white tw:text-neutral-600 tw:hover:border-neutral-300",
    },
    {
      color: "green",
      isSelected: false,
      isDisabled: false,
      className:
        "tw:border-green-200 tw:bg-green-100 tw:text-green-700 tw:hover:border-green-300",
    },
    {
      color: "yellow",
      isSelected: false,
      isDisabled: false,
      className:
        "tw:border-yellow-200 tw:bg-yellow-100 tw:text-yellow-700 tw:hover:border-yellow-300",
    },
  ],
  defaultVariants: {
    color: "gray",
...
export function TagGroup<T extends object>({
  ...
  return (
    ...
    >
      ...
      {errorMessage && (
        <Text className="tw:text-sm tw:text-red-600" slot="errorMessage">
          {errorMessage}
        </Text>
      )}
    </AriaTagGroup>
...
export function Tag({ color, ...otherProps }: TagProps) {
  return (
    <AriaTag
      textValue={
      ...
      className={composeRenderProps(
        ...
        (className, renderProps) =>
          tagStyles({ ...renderProps, className, color }),
      )}
    >
      {composeRenderProps(
        otherProps.children,
          ...
          <>
            ...
            {allowsRemoving && (
              <Button className={removeButtonStyles} slot="remove">
                <XIcon aria-hidden className="tw:h-3 tw:w-3" />
              </Button>
            )}
          </>
        ),
